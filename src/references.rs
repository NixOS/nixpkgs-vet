...
use crate::problem::{npv_121, npv_122, npv_123, npv_124, npv_125, npv_126, npv_127};
    ...
    Ok(validation::sequence_(
        nix_file.syntax_root.syntax().descendants().map(|node| {
            ...
            match nix_file.static_resolve_path(&path, absolute_package_dir) {
                ...
                .into(),
                ResolvedPath::RelativeFromPathError(err) => {
                    npv_127::PackageContainsInvalidReference::new(
                        relative_package_dir,
                        subpath,
                        line,
                        text,
